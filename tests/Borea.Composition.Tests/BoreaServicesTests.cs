...
public sealed class BoreaServicesTests : IDisposable
{
    ...
    public async Task Mods_IsTheCompositeRepository()
    {
        ...
        Assert.IsType<FileModInstaller>(Assert.IsType<CheckedModInstaller>(Assert.IsType<LoggingModInstaller>(services.Installer).Inner).Inner);
        Assert.IsType<FileModReplacer>(Assert.IsType<CheckedModReplacer>(Assert.IsType<LoggingModReplacer>(services.Replacer).Inner).Inner);
        ...
        Assert.IsType<FileGameSaveBackupStore>(services.GameSaveBackups);
    }
